...
import com.mars.domain.PersistentToken;
import com.mars.domain.User;
import com.mars.repository.PersistentTokenRepository;
import com.mars.repository.UserRepository;
import com.mars.security.SecurityUtils;
import com.mars.service.MailService;
import com.mars.service.UserService;
import com.mars.service.dto.UserDTO;
import com.mars.web.rest.vm.KeyAndPasswordVM;
...
public class AccountResource {
	...
	public ResponseEntity<String> saveAccount(@Valid @RequestBody UserDTO userDTO) {
		...
		return userRepository
			.findOneByLogin(SecurityUtils.getCurrentUser())
			.map(u -> {
	 ...
	 *
	 * @return the ResponseEntity with status 200 (OK) and the current open sessions in body, or status 500 (Internal
	 * Server Error) if the current open sessions couldn't be retrieved
	 */
	@GetMapping("/account/sessions")
	@Timed
	public ResponseEntity<List<PersistentToken>> getCurrentSessions() {
		return userRepository.findOneByLogin(SecurityUtils.getCurrentUser())
	...
	public void invalidateSession(@PathVariable String series) throws UnsupportedEncodingException {
		...
		userRepository.findOneByLogin(SecurityUtils.getCurrentUser()).ifPresent(u -> {
